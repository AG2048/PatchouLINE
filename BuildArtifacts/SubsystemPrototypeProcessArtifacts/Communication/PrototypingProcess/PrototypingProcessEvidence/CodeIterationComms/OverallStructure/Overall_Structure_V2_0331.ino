...
void button1(WiFiClient client) {
  ...
  client.println("}");
  client.println("};");
  client.println("xhttp.open('GET', '/button1pressed', true);");
...
// ok this might not work for the actual code bc i will have info after the /function name
// I THINK THIS IS WHERE I DO THE READING!!!! '\r'!!!!!

        if (currentLine.endsWith("GET /button1pressed")) {
           Serial.println("BUTTON ONE RECEIVED_----------------------------");
           state = 1;
        }
        if (currentLine.endsWith("GET /button2pressed")) {
           Serial.println("BUTTON TWO RECEIVED_----------------------------");
           state = 2;
        }
      }
    }
