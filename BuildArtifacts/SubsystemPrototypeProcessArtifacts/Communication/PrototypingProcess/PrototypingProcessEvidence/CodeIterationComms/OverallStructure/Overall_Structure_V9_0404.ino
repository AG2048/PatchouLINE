...
current version: v9
working on: formatting, include sensor deployment code, testing main loop with GUI print
*/
  ...
  while (!deploymentFinish) {
    ...
    switch (currentState) {
      ...
      {      
        ...
        roundedReading = round(moistureReading);        //i added this
        Serial.println(roundedReading);                     //what is tot? 
...
void loop() {
  ...
  //ok i think i need to restructure the code so that kevin's stuff can run outside of this case statement bc that will not work
  //it wont go back into the cases UNLESS there is interaction with the client which is the problem...
  //solution: booleans lol just one separate part for the section of inputs and then "else" aka some boolean is false means run kevin's stuff
  //or i do bigger case statement... sad
  //that means i will need to do this whole "if client" stuff again outside somewhere else
  ...
  case 1:
    ...
    // m_retrieved = kevinCode();
    //it should break out of his loop when it's done one "cycle" and wait until ready to deploy again
    //that means the variables of new_input are ready for storage
    //i switched it to an int function, hopefully still works properly
    ...
    //run the GUI
    printGUI(client);
    //the issue is if i run it like this, it just keeps adding it to the webpage
    //like inserting identical heatmaps underneath infinitely
    //for some reason without the if thing it prints headers???? what?????? im so confused lol
    //i have NOT ADDED THE LOOP YET
    ...
    delay(8000); //for my sanity so this loops and it is fine i set it to do that for now anyways
