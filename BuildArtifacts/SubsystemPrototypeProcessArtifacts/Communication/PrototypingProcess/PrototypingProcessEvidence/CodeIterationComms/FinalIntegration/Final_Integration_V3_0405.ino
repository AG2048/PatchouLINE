...
current version: V3
working on: remove serial check
*/
...
void deploySensorProcess() {
  ...
  switch (currentState) {
    ...
    case EXTENDING:
    {
      ...
      if(digitalRead(LIM_SWITCH_BOT)){ currentState = EXTENDED; break;}
    ...
    case RETRACTING:
    {
      ...
      if(digitalRead(LIM_SWITCH_TOP)){ currentState = RETRACTED; break;}
...
void setup() {
  ...
  // this was removed because we are not using serial port for the prototype.
  // trying to be more efficient and use less lines of code, hopefully makes it faster
