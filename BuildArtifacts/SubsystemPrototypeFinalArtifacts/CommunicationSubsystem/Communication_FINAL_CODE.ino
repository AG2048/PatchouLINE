/*
FINAL VERSION
last update: april 7th, 2024
current version: V7

The following is the Communications Subsystem for the PatchouLine Praxis III Project.

The code consists of two portions as defined by case statements within the main loop:
- the initial user input from the webpage
- the retrieval of data from other subsystems and displaying to the webpage
The Wi-Fi on the Arduino Nano RP2040 Connect was used to facilitate wireless connectivity between users and the project.

Multiple functions were defined to assist with sending information between HTML/JavaScript and Arduino
HTML was used to create initial pages for users to input data through
The Sensor Subsystem's code was also integrated through functions and called in the second half of the loop
Finally, a heatmap was projected and continuously updates as new sensor data is sent in.

Indonesian translations have been added to all webpage outputs for stakeholder consideration.
Descriptions of the heatmap are also added to increase usability and comprehension of the webpage.
*/
...
// to jump between case statements
int big_state = 0;    //main sections of code
int state = 0;        //states within the user input
...
// ignore manual mode for the prototype.
...
int storage[2][2][1];  //will be a maximum size defined by designers.
//the prototype is only a two by two grid, and only retrieves moisture data (1 value)
//actual dimensions will be storage[140][140][4] as defined by average plot sizes in Indonesia.
...
nutrients store() {
// would be received from the sensor team, but only moisture will be defined.
// this function is not used when prototyping, but has been written for our understanding of the final design.
  ...
  int x_new = x / (x_spacing_value/100);        //to adjust the x values to integers for indexing
...
void processInitInput(WiFiClient client) {
  ...
  client.println("<h1 style='color: #5F9EA0'> <i>Parameter Peternakan Nilam</i></h1>");

  //assuming min 0.6 apart so max 84*0.6 = 140 rows
  client.print("<div class='slideContainer'>");
  client.println("<h2>Plot Width / <i>Lebar Petak </i>(X): <span id='x_width'>42</span></h2>");                               
  client.println("0<input type='range' min='0' max='84' value='42' id='slider1' list='XYmarkers' />84");  // slider for x width

  client.println("<h2>Plot Length / <i>Panjang Petak </i>(Y): <span id='y_length'>42</span></h2>");
  client.println("0<input type='range' min='0' max='84' value='42' id='slider2' list='XYmarkers' />84");  // slider for y length

  client.println("<h2>Number of Rows / <i>Jumlah Baris</i>: <span id='num_rows'>70</span></h2>");
  client.println("0<input type='range' min='0' max='140' value='70' id='slider3' list='NUMmarkers' />140");  // slider for number of rows

  //from research, average plot distance is 60cm which corresponds to our 140 maximum plants per row
  client.println("<h2>X Spacing / <i>X Spasi </i>[cm]: <span id='x_spacing'>100</span></h2>");
  client.println("60<input type='range' min='60' max='120' value='100' id='slider4' list='NUMmarkers' />120");  // slider for x distance

  client.println("<h2>Mode (Automatic or Manual) / <i>Mode (Otomatis atau Manual) </i>: <span id='modeAM'>N</span></h2>");
  ...
  client.println("<button class = 'buttonStyle' onclick='sendSliderValues()'>SUBMIT<br> <i>KIRIM</></button>");
  client.println("</div>");

  //call the function to extract the values when the button has been pressed
  ...
  //modes are not relevant for the prototype as we are only doing automatic mode.
  ...
  client.println("}");
  client.println("document.getElementById('refreshMessage').innerHTML = 'Please refresh the screen! / Harap segarkan layar!';");
  client.println("}");
  client.println("};");
  // Send all five slider values to the server, this is the "url" separating values with delimiters "=" and "&" for easier parsing
...
void processAutoInput(WiFiClient client) {
  ...
  client.println("<h1 style='color: #5F9EA0'>Automatic Mode Parameters /<i> Parameter Mode Otomatis </i> </h1>");

  //assuming min 0.6 apart so max 84*0.6 = 140 rows
  client.print("<div class='slideContainer'>");
  client.println("<h2>X Distance Between Plants / <i> X Jarak Antar Tanaman</i> [m]: <span id='x_dist'>42</span></h2>");                                       //THIS IS THE PART SEEN ON THE SCREEN
  ...
  client.println("<h2>Row Frequency /<i>Frekuensi Baris</i>: <span style='font-family: Tahoma' id='row_dist'>70</span></h2>");
  ...
  // need to serial print these values
  client.println("var xhttp = new XMLHttpRequest();");
  client.println("xhttp.onreadystatechange = function() {");
  client.println("if (this.readyState == 4 && this.status == 200) {");
  //update the text with the new values
  ...
  client.println("document.getElementById('refreshMessage').innerHTML = 'Please refresh the screen! / Harap segarkan layar!';"); //tell users to refresh
  client.println("}");
  client.println("};");
...
// heatmap function to be printed to the webpage
      ...
      if (client.available()) {    // if there's bytes to read from the client,
        ...
        if (c == '\n') {           // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.println("        html,");
            client.println("        body,");
            client.println("        #container {");                     //general container parameters for aesthetics
            ...
            client.println("        }");
            client.println("");
            client.println("        .heatmap {");                       //sizing of heatmap is 2x2 for our prototype
            ...
            client.println("        }");
            client.println("");     //these are all definitions for the appearance of the heatmap.
            client.println("        .plot {");
            ...
            client.println("");
            client.println("<body>");
            //ALERT for when new data has been added, so that users know which plot was changed.
            if (newData) {
              client.println("<h1 style='color: #5F9EA0'><center>Patchouli Soil Moisture Levels -- New data at plot location (" + String(plot_index/2 +1) + "," + String(plot_index%2 +1) + ")</center></h1>");
              newData = false;
            } else client.println("<h1 style='color: #5F9EA0'><center>Patchouli Soil Moisture Levels</center></h1>");

            client.println("    <!--Title-->");
            client.println("    <center>");
            //client.println("        <h1> Patchouli Soil Moisture Levels</h1>");
            client.println("        <h2 style='color: #5F9EA0'><i>Tingkat Kelembaban Tanah Nilam</i></h2>");
            client.println("    </center>");
            client.println("");
            client.println("");
            //text descriptions of how the heatmap works in english and indonesian
            client.println("    <body> The following map depicts the soil moisture levels of all patchouli plots. Each plot is marked with a colour which represents its soil's moisture level. The numbers along the rows and columns distinguish each plot according to the dimensions specified in the previous page. Please see the legend below to determine the soil moisture level of the plot and the associated recommendation.</body>");
            client.println("    <br>");
            client.println("    <br>");
            client.println("    <body><i>Peta berikut menggambarkan tingkat kelembaban tanah di seluruh petak nilam. Setiap plot ditandai dengan warna yang mewakili tingkat kelembaban tanah. Angka-angka pada baris dan kolom membedakan setiap plot sesuai dengan dimensi yang telah ditentukan pada halaman sebelumnya. Silakan lihat legenda di bawah ini untuk menentukan tingkat kelembaban tanah di plot dan rekomendasi terkait.</i></body>");
            client.println("    <br>");
            client.println("    <br>");
            client.println("    <body> <strong>Click on a coloured square to receive an update of the region's soil status!</strong></body>");
            client.println("    <br>");
            client.println("    <br>");
            client.println("    <body><i><strong>Klik pada kotak berwarna untuk menerima pembaruan status tanah di wilayah tersebut!</i></strong></body>");
            ...
            //Explanations for the legend and what that means for moisture
            ...
            client.println("    <br>");
            client.println("     <br>");
            client.println(""); //descriptions of recommendations for moisture levels based on calibration
            ...
            // Generate heatmap, converting the moistureLevels array that contains new sensor data into JavaScript for the HTML to read
            ...
            // Generate heatmap
            client.println("            for (let i = 0; i < xCoord * yCoord; i++) {"); //read the array to get each element
            ...
            client.println("                plot.dataset.heat = 'Soil Moisture Level: ' +  Math.round(moistureLevel/850*100) + '% \\n Tingkat Kelembaban Tanah: ' +  Math.round(moistureLevel/850*100) + '%' ;");
            client.println("");
            ...
            // should change the plot reading every refresh

            moistureLevels[plot_index] = count;
            count = (count + 100)%850;           //calibration
            newData = true;
            plot_index = (plot_index+1)%4;       //keep track of plot location
...
// only for displaying the storage array if a test is needed
void testprint() {
  for (int i = 0; i < 4; i++) {
...
void loop() {
  ...
  switch (big_state) { //this is to separate between input and output (retrieve farmer data / output sensor data)
    case 0: //inner start case for retriving farmer data
          ...
          if (client.available()) {    // if there's bytes to read from the client,
              ...
              if (currentLine.length() == 0) {
                ...
                switch (state) {
                  case 0:  // initial page, farmer plot parameters
                    ...
                    processInitInput(client); //call the HTML
                    Serial.println(state);
                    break;
                  case 1:  // automatic case page
                    Serial.println("enter case 1");
                    processAutoInput(client); //call the HTML
                    Serial.println(state);
                    break;
                  case 2:  //temporary case to make sure it does not loop back into case 1. so fast it won't be shown, just activate next state 
                    ...
                    big_state = 1; //switch over to sensor deployment section
                }
                client.println();
                break;
              } else {  // if you got a newline, then clear currentLine:
            ...
            if ((currentLine.startsWith("GET /inputSliders")) && (currentLine.endsWith("HTTP/1.1"))) {
              ...
              if (currentLine.indexOf("GET /inputSliders?") != -1) {
                ...
                // testing that all correct values are printed
                Serial.print("exit retrieving input values (input screen 1)_____________________");
                Serial.print("\n x_width_value: " + x_width_value);
                Serial.print(" \n y_length_value: " + y_length_value);
                Serial.print("\n num_rows_value: " + num_rows_value);
                Serial.print("\n x_spacing_value: " + x_spacing_value);
            ...
            if ((currentLine.startsWith("GET /autoSliders")) && (currentLine.endsWith("HTTP/1.1"))) {
              ...
              state = 2; //move to display screen before the next case
              Serial.println(currentLine);
              if (currentLine.indexOf("GET /autoSliders?") != -1) {
                ...
                // testing that all correct values are printed
                Serial.print("exit retrieving input values (input screen 2)_____________________");
                Serial.print("\n x_dist_value: " + x_dist_value);
                Serial.print("\n row_dist_value: " + row_dist_value);
                Serial.print("\n num_rows_value: " + num_rows_value);
              }
            }
          }
    ...
    case 1: //this is the case that will now continously loop: trigger sensor deployment and update heatmap

      Serial.println("enter sensor retrieval and display code");

      deploySensorProcess();
      //it should break out of sensor deployment loop when it's done one "cycle" and wait until ready to deploy again

      //run the GUI with the heatmap
      printGUI(client);

      //test that the correct values are being outputted to monitor in the serial
      testprint();

      //no longer needed for the prototype, but would be located here for the final design
...
// initial function to print when the Wi-Fi has been set up
